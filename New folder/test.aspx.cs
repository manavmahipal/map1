...
namespace merit
{
    ...
    {
        //string conn_str = "Server=127.0.0.1;Uid=root;Database=commercial;Pwd=Manav@009944";
        ...
        protected void generate_table_chart(string date_filter_input)
        {
            ...
            try
            {
                ...
                cmdText = "SELECT MIN(a.VC) AS VC, a.StationID, b.display_name, b.Utility_subtype FROM mo_master01 a, station_master b WHERE ((a.date_from <= '" + date_filter_input + "' AND a.date_to >= '" + date_filter_input + "') OR (a.date_from <= '" + date_filter_input + "' AND a.date_to IS NULL)) AND a.StationID = b.SrNo AND a.VC > 0 GROUP BY a.StationID ORDER BY VC ";
