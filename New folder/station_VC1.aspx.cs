...
namespace merit
{
    public partial class station_VC1 : System.Web.UI.Page
    {
        ...
        protected void VC_filter_DDL_change(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                string cmdText = "SELECT SrNo,StationName FROM station_master WHERE SrNo IN (SELECT DISTINCT stationID FROM mo_master01 WHERE stateID=" + Convert.ToInt32(DDL_for_VC_filter.SelectedItem.Value) + ") ORDER BY StationName";
        ...
        protected void generate_table_chart(string station_input,string filter_input)
        {
            ...
            try
            {
                ...
                if (filter_input == "Min")
                    ...
                    cmdText = "SELECT a.display_name, b.date_from,b.date_to,MAX(b.VC) AS VC FROM station_master a,mo_master01 b WHERE a.SrNo = b.StationID AND a.SrNo = " + Convert.ToInt32(station_input) + " AND(b.VC <> 0 OR b.VC IS NOT NULL OR b.VC < 20) AND b.StateID = " + Convert.ToInt32(filter_input) + " GROUP BY b.date_from ORDER BY b.date_from ASC";
                MySqlCommand cmd = new MySqlCommand(cmdText, con);
                MySqlDataAdapter adapter = new MySqlDataAdapter();
                adapter.SelectCommand = new MySqlCommand(cmdText, con);
                adapter.Fill(dt);
        ...
        protected void populate_ddl(string stateID_input)
        {
           // string str = @"Server=MYSQL5044.site4now.net;Database=db_a70e2a_mydata;Uid=a70e2a_mydata;Pwd=Rimmi@1402";
            ...
            {
                con.Open();
                string cmdText = "SELECT SrNo,StationName FROM station_master WHERE SrNo IN(SELECT DISTINCT stationID FROM mo_master01 WHERE stateID = " + Convert.ToInt32(stateID_input) + ") ORDER BY StationName ASC";
                MySqlCommand cmd = new MySqlCommand(cmdText, con);
                MySqlDataAdapter adapter = new MySqlDataAdapter();
                adapter.SelectCommand = new MySqlCommand(cmdText, con);
                adapter.Fill(dt);
